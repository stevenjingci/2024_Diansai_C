...
{ 
	...
	__HAL_RCC_GPIOB_CLK_ENABLE();	 //PA,PB,PC端口时钟使能
	...
	HAL_GPIO_Init(GPIOC, &GPIO_InitStructure);					 //根据设定参数初始化GPIOA
	
	GPIO_InitStructure.Pin = GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_11;//初始化管脚PB0.1.10			
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP; 		 //推挽输出
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_VERY_HIGH;		 //IO口速度为2MHz
	HAL_GPIO_Init(GPIOD, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB
	
	
	GPIO_InitStructure.Pin = GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7|GPIO_PIN_8;	//初始化管脚PC0			 
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP; 		 //推挽输出
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_VERY_HIGH;		 //IO口速度为2MHz
	HAL_GPIO_Init(GPIOG, &GPIO_InitStructure);					 //根据设定参数初始化GPIOC
...
/***************************************2024C************************************************/
/*SD:CH1:CW,CH2:AM*/   /*SM:CH0:CW,CH3:AM*/
void mw_set_frequency(void)
{
	AD9959_Set_Fre(CH2, 2000000);	
	AD9959_Set_Fre(CH3, 2000000);	
}
void mw_am_param(void)
{
	AD9959_Set_Amp(CH2, Signal.Ma*11.2); 		
	AD9959_Set_Amp(CH3, Signal.Ma*11.2); 

}
void cw_set_frequency(void)
{
	AD9959_Set_Fre(CH0, Signal.fc);
	AD9959_Set_Fre(CH1, Signal.fc);
}
float amplification_param1,amplification_param2,sm_reduction_param;
void cw_set_amptitude(void)
{
	
	amplification_param1=(100-1.6*(40000000-Signal.fc)/1000000)/100.0;
	amplification_param2=0.966*pow(0.989,(1000-Signal.cw_rms)/100)*(pow(0.995,(40000000-Signal.fc)/1000000));
	sm_reduction_param=pow(10,Signal.reduction_dB/-20.0);
	AD9959_Set_Amp(CH0, Signal.cw_rms*amplification_param1*sm_reduction_param); 	////****////	
	AD9959_Set_Amp(CH1, Signal.cw_rms*amplification_param2);   
}
void phase_setting(void)
{
	uint16_t SM_CW_phase,SM_MW_phase;
	SM_MW_phase=(int)(360-10-Signal.phase_difference+Signal.delay_ns*360*0.002)%360;
	SM_CW_phase=(int)(360-10-Signal.phase_difference+Signal.fc/1000000000.0*Signal.delay_ns*360)%360;
	//SD
	AD9959_Set_Phase(CH1, Signal.phase_sd);	//CW
	AD9959_Set_Phase(CH2, Signal.phase_sd);	//MW
	//SM
	AD9959_Set_Phase(CH0, SM_CW_phase/360.0*16383);	//CW		
	AD9959_Set_Phase(CH3, SM_MW_phase/360.0*16383);	//MW
	
//	uint8_t string_uart[50];
//	sprintf((char*)string_uart,"SM_MW_phase:%d\nSM_CW_phase:%d\n",SM_MW_phase,SM_CW_phase);
//	HAL_UART_Transmit(&huart1,string_uart,strlen((char*)string_uart),50);
